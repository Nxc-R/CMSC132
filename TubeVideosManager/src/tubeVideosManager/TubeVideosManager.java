...
public class TubeVideosManager implements TubeVideosManagerInt {
	...
	public ArrayList<Video> getAllVideosInDB() {
		return new ArrayList<Video>(videoDatabase);
	}
	...
	public boolean addComments(String title, String comments) {
		...
		for (int i = 0;i < videoDatabase.size();i++) {
			if (findVideo(title) != null && 
					videoDatabase.get(i).equals(findVideo(title))) {
	...
	public boolean addVideoToPlaylist(String title, String playlistName) {
		if (title == null || title.isBlank() || playlistName == null || playlistName.isBlank()) {
			throw new IllegalArgumentException("Invalid Parameters");
		}
		boolean exists = false;
		int index = -1;
		...
		if (exists) {
			playlists.get(index).addToPlaylist(title);
			return true;
		} else {
			return false;
		}
	...
	public Playlist searchForVideos(String playlistName, String title, int maximumDurationInMinutes, Genre genre) {
		Playlist returnPlaylist = new Playlist(playlistName);
		if (title == null && maximumDurationInMinutes == -1 && genre == null) {
			for (int i = 0;i < videoDatabase.size();i++) {
				returnPlaylist.addToPlaylist(videoDatabase.get(i).getTitle());
			}
		}
		if (!(title == null) && maximumDurationInMinutes > -1 && !(genre == null)) {
			for (int i = 0;i < videoDatabase.size();i++) {
				Video temp = videoDatabase.get(i);
				if (temp.getTitle().equals(title) && temp.getDurationInMinutes() 
						<= maximumDurationInMinutes && temp.getGenre() == genre) {
					returnPlaylist.addToPlaylist(temp.getTitle());
				}
			}
		}
		for (int i = 0;i < playlists.size();i++) {
			...
			if (title == null && maximumDurationInMinutes > -1 && genre == null) {
				if (temp.getDurationInMinutes() <= maximumDurationInMinutes) {
					returnPlaylist.addToPlaylist(videoDatabase.get(i).getTitle());
				}
			}
			if (title == null && maximumDurationInMinutes == -1 && !(genre == null)) {
